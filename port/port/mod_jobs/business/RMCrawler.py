...
class RMCrawler(JobCrawler):
    def __init__(self, keyword=''):
        return super(RMCrawler, self).__init__(keyword=keyword, country='RM')
    ...
    def GetReed(self, islocal=False, issave=False): 
        self.dk.AddItems(self.ParseReed(islocal=islocal,issave=issave))
    ...
    def ParseReed(self, islocal=False, issave=False): 
        host='landing.jobs'
        if islocal:
            html=self.GetFile(host+str(n))
        else:
            html=self.GetPage(host, 'https://landing.jobs/jobs?page=1&q=%s&remote=true&hd=false&t_co=false&t_st=false' % (self.keyword))
            if issave:
